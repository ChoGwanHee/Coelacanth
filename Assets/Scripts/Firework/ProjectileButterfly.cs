...
public class ProjectileButterfly : BaseProjectile
{
    ...
    private void Explosion()
    {
        ...
        for (int i = 0; i < effectedObjects.Length; i++)
        {
            ...
            if (effectedObjects[i].CompareTag("Player"))
            {
                ...
                if (!effectedPlayer.PC.isUnbeatable)
                {
                    ...
                    objPhotonView.RPC("DamageShake", objPhotonView.owner, damage, 4, photonView.ownerId);
        ...
        PhotonNetwork.Instantiate("Prefabs/nabi_firework_Hit_fx", fixedPosition, transform.rotation, 0);
